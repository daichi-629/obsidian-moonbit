...
export function createEmbeddedMoonBitWasmModule(
	module: EmbeddedMoonBitWasmModule
): EmbeddedMoonBitWasmModule {
	return module;
}

export async function loadEmbeddedMoonBitWasmModule<
	TExports extends WebAssembly.Exports = WebAssembly.Exports
>(
	module: EmbeddedMoonBitWasmModule,
	options: LoadEmbeddedMoonBitWasmModuleOptions
): Promise<TExports> {
	assertEmbeddedMoonBitWasmModule(module);
	const cachePath = buildMoonBitWasmCachePath(options.cacheRoot, module);
...
export function buildMoonBitWasmCachePath(
	cacheRoot: string,
...
function assertEmbeddedMoonBitWasmModule(module: EmbeddedMoonBitWasmModule): void {
	if (module.kind !== "embedded-moonbit-wasm-module") {
		throw new Error("Expected an embedded MoonBit wasm module artifact");
	}
}
